...
References to method names in `CancelInvoke` and `IsInvoking` are also recognized.
...
`InvokeMe` is called indirectly by Unity, or referenced by name when checking or cancelling a pending invocation, which is undetectable by the IDE.
